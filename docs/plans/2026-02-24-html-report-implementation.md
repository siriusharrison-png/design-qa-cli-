...
**Goal:** 为 design-qa-cli 添加 HTML 报告输出功能，生成包含统计卡片、颜色对比、数据导出的可视化报告。

**Architecture:** 新增 `html-reporter.js` 模块生成 HTML 内容，修改 CLI 添加 `--output` 参数。报告为单文件 HTML，内联所有 CSS 和 JS。

**Tech Stack:** Node.js、ES Modules、原生 HTML/CSS/JS

---

### Task 1: 创建 HTML 报告模块骨架

**Files:**
- Create: `src/html-reporter.js`
...
Run: `cd ~/Desktop/design-qa-cli && node -e "import('./src/html-reporter.js').then(m => console.log('OK'))"`
Expected: `OK`

**Step 3: Commit**

```bash
git add src/html-reporter.js
git commit -m "feat: add html-reporter module skeleton"
```
...
**Files:**
- Modify: `src/html-reporter.js`

**Step 1: 实现 getStyles 函数**
...
```bash
git add src/html-reporter.js
git commit -m "feat: add CSS styles for html report"
```
...
```bash
git add src/html-reporter.js
git commit -m "feat: add header and summary cards"
```
...
```bash
git add src/html-reporter.js
git commit -m "feat: add export buttons"
```
...
```bash
git add src/html-reporter.js
git commit -m "feat: add issues list with color comparison"
```
...
```bash
git add src/html-reporter.js
git commit -m "feat: add export JSON/CSV scripts"
```

---

### Task 7: 修改 CLI 添加 --output 参数
...
在 `printReport(issues, colors);` 之后添加：
...
```bash
git add bin/cli.js
git commit -m "feat: add --output option for HTML report"
```
...
Run: `cd ~/Desktop/design-qa-cli && node bin/cli.js check ./test --output test-report.html`

Expected: 终端输出检测结果 + "HTML 报告已生成: test-report.html"

**Step 2: 在浏览器中打开报告**

Run: `open ~/Desktop/design-qa-cli/test-report.html`

Expected: 浏览器显示报告页面，包含统计卡片、问题列表、颜色对比

**Step 3: 测试导出功能**

在报告页面点击"导出 JSON"和"导出 CSV"按钮，验证下载功能正常。

**Step 4: 清理测试文件**

```bash
rm ~/Desktop/design-qa-cli/test-report.html
```
...
```bash
git add .
git commit -m "feat: complete HTML report feature"
```
